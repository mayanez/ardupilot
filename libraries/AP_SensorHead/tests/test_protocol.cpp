...
TEST_F(ProtocolTest, decode) {
    ...
    bool decoded =
        Packet::decode(&a_packet, &e_raw_offset_garbage[0],
                       static_cast<uint16_t>(sizeof(e_raw_offset_garbage)));

    EXPECT_FALSE(decoded);
    // Move past garbage. In actual use, moving past this garbage will depend on
    // transport layer.
    decoded = Packet::decode(&a_packet, &e_raw_offset_garbage[2],
                             static_cast<uint16_t>(sizeof(e_raw_offset_garbage)));
    EXPECT_TRUE(decoded);
